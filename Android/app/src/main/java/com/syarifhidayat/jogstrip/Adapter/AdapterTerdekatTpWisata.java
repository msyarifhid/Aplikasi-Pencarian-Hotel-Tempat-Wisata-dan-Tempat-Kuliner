...
public class AdapterTerdekatTpWisata extends RecyclerView.Adapter<AdapterTerdekatTpWisata.ViewwHolder> {

    Context context;
    List<BasePlace> listBasePlace;

    public AdapterTerdekatTpWisata(Context context, List<BasePlace> listBasePlace) {
    ...
    public ViewwHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new ViewwHolder(LayoutInflater.from(context).inflate(R.layout.layout_item_terdekat_tpwisata, parent, false));
    ...
    public void onBindViewHolder(@NonNull ViewwHolder holder, int position) {
        ...
        holder.tv_jarakTerdekatTpWisata.setText(new DecimalFormat("#.##").format(bP.getJarakTerdekat())+ " KM");
        holder.tv_tempatTerdekatTpWisata.setText(bP.getNamaTerdekat());

        if (bP.getJenis().equals("hotel")){
            holder.icTerdekatTpWisata.setColorFilter(Color.RED, PorterDuff.Mode.SRC_ATOP);
        }else if(bP.getJenis().equals("tempat_wisata")){
            holder.icTerdekatTpWisata.setColorFilter(Color.GREEN, PorterDuff.Mode.SRC_ATOP);
        }else if(bP.getJenis().equals("tempat_kuliner")){
            holder.icTerdekatTpWisata.setColorFilter(Color.YELLOW, PorterDuff.Mode.SRC_ATOP);
        }

        //click to deskripsiTerdekat
        holder.ly_itemTerdekatTpWisata.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    static class ViewwHolder extends RecyclerView.ViewHolder{
        ...
        ImageView icTerdekatTpWisata;
        TextView tv_tempatTerdekatTpWisata, tv_jarakTerdekatTpWisata;
        LinearLayout ly_itemTerdekatTpWisata;

        public ViewwHolder(View itemView) {
            super(itemView);

            icTerdekatTpWisata = itemView.findViewById(R.id.icTerdekatTpWisata);
            tv_tempatTerdekatTpWisata = itemView.findViewById(R.id.tv_tempatTerdekatTpWisata);
            tv_jarakTerdekatTpWisata = itemView.findViewById(R.id.tv_jarakTerdekatTpWisata);
            ly_itemTerdekatTpWisata = itemView.findViewById(R.id.ly_itemTerdekatTpWisata);
        }
    }
}
